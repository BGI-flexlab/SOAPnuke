...
peProcess::peProcess(C_global_parameter m_gp)
{ // initialize
	gp = m_gp;
	...
	for (uint64_t i = 0; i < gp.threads_num; i++)
	{
		...
		local_trim_stat1[i] = C_fastq_file_stat(gp);
		local_trim_stat2[i] = C_fastq_file_stat(gp);
		local_clean_stat1[i] = C_fastq_file_stat(gp);
		local_clean_stat2[i] = C_fastq_file_stat(gp);
	}
...
void peProcess::print_stat()
{ // print statistic information to the file
...
void peProcess::update_stat(C_fastq_file_stat &fq1s_stat, C_fastq_file_stat &fq2s_stat, C_filter_stat &fs_stat, string type)
{ // update statistic information from each thread
	if (type == "raw")
	{
...
void *peProcess::stat_pe_fqs(PEstatOption opt, string dataType)
{ // statistic the pair-ends fastq
	...
	for (vector<C_fastq>::iterator ix = opt.fq2s->begin(); ix != ix2_end; ix++)
	{
		...
		for (string::size_type i = 0; i != seq_len; i++)
		{
			...
			case 'a':
			case 'A':
				opt.stat2->bs.position_acgt_content[i][0]++;
				opt.stat2->gs.a_number++;
				break;
			case 'c':
			case 'C':
				opt.stat2->bs.position_acgt_content[i][1]++;
				opt.stat2->gs.c_number++;
				break;
			case 'g':
			case 'G':
				opt.stat2->bs.position_acgt_content[i][2]++;
				opt.stat2->gs.g_number++;
				break;
			case 't':
			case 'T':
				opt.stat2->bs.position_acgt_content[i][3]++;
				opt.stat2->gs.t_number++;
				break;
			case 'n':
			case 'N':
				opt.stat2->bs.position_acgt_content[i][4]++;
				opt.stat2->gs.n_number++;
				break;
			default:
			{
				cerr << "Error:unrecognized sequence," << ((*ix).sequence) << endl;
...
void peProcess::preOutput(int type, C_fastq &a)
{ // modify the sequences before output if necessary
...
void peProcess::peWrite(vector<C_fastq> &pe1, vector<C_fastq> &pe2, gzFile out1, gzFile out2)
{ // output the sequences to  files
...
int peProcess::read(vector<C_fastq> &pe1, vector<C_fastq> &pe2, ifstream &infile1, ifstream &infile2)
{ // read file from ASCII text,used in ssd mode
