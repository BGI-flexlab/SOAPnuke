...
class SOAPnuke(cmd.Cmd):
    ...
    def args_streaming(*parser):  # The default paras below should be ensured correct.
        ...
        parser.add_argument("--hadoop", help="hadoop Path",          type=str, default='usr/bin/hadoop')
        parser.add_argument("--jar",    help="Streaming_fq.jar",     type=str, default='./Streaming_fq.jar')
        parser.add_argument("--java",   help="java Path",            type=str, default='usr/bin/java')
        parser.add_argument("--R",      help='RPath',                type=str, default='usr/bin/R')
        parser.add_argument("--jn", dest='jobname',  help='jobname', type=str, default='SOAPnuke_streaming')
        ...
        parser.add_argument("--rm", dest='reducememory', help='memory for reduce_tasks, unit=MB',   type=int)
        parser.add_argument("--lj", dest='libjars',      help='SuffixMultipleTextOutputFormat.jar', type=str,
                            default='./SuffixMultipleTextOutputFormat.jar')
        parser.add_argument("-i", "--input",             help='input_directory',                    type=str)
        ...
        parser.add_argument("--merge",                   help='whether merge fastq pieces or not',  type=int)
        return parser
    ...
    def args2cmd(arg, module):
        ...
        args = '''%s jar %s -D mapred.job.name="%s"
                    -D mapred.map.tasks=%s -D mapred.reduce.tasks=%s -D mapreduce.reduce.memory.mb=%s
               ...
               % (args1['hadoop'],       args1['jar'],     args1['jobname'], args1['maptasks'],    args1['reducetasks'],
                  args1['reducememory'], args1['libjars'], args1['input'],   args1['outdir_hdfs'], module, args2)
    ...
    def merge(args):
        ...
        # R
        if not pe_mark:
            os.system('%s/Rscript/base.R %s/Base_distribution_by_read_position_1.txt '
                      '%s/Base_distribution_by_read_position_raw_1.png '
                      '%s/Base_distribution_by_read_position_clean_1.png')
            os.system('%s/Rscript/base.R ')
