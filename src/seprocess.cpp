...
void seProcess::filter_se_fqs(SEcalOption opt){
    ...
    for(vector<C_fastq>::iterator i=opt.fq1s->begin();i!=opt.fq1s->end();i++){
        ...
        if(gp.adapter_discard_or_trim=="trim"||gp.contam_discard_or_trim=="trim"||!gp.trim.empty()
           ||!gp.trimBadHead.empty()||!gp.trimBadTail.empty()){
...
void seProcess::addCleanList(int tmp_cycle,int index){
    ...
    if(find(readyCleanFiles1[index].begin(),readyCleanFiles1[index].end(),checkClean1.str())
       ==readyCleanFiles1[index].end()){
    ...
    if(!gp.trim_fq1.empty()){
        ...
        if(find(readyTrimFiles1[index].begin(),readyTrimFiles1[index].end(),readyTrimR1.str())
           ==readyTrimFiles1[index].end())
                ...
                for(int cycle=cur_cat_cycle;cycle<ready_cycles;cycle++){
                    for(int i=0;i<gp.threads_num;i++){
                        ...
                        if(cur_avaliable_total_reads_number>=gp.cleanOutSplit){
                            int toBeOutputReadsNumber=gp.cleanOutSplit-(
                                    cur_avaliable_total_reads_number-clean_file_readsNum[i][cycle]
                            );
                            extractReadsToFile(
...
void seProcess::run_extract_random(){
    ...
    string out1=gp.cleanOutGzFormat?gp.output_dir+"/cleanRandomExtractReads.r1.fq.gz":
            gp.output_dir+"/cleanRandomExtractReads.r1.fq";
...
void seProcess::seStreaming_stat(C_global_variable &local_gv){
    ...
    int total=local_gv.fs.include_adapter_seq_num+local_gv.fs.include_contam_seq_num+local_gv.fs.low_qual_base_ratio_num
              +local_gv.fs.mean_quality_num+local_gv.fs.n_ratio_num+local_gv.fs.over_lapped_num+local_gv.fs.highA_num
              +local_gv.fs.polyX_num;
...
//    int thread_cycle=-1;
    ...
    if(inputGzformat){
        while(1){
            if(gzgets(
                    multi_gzfq1[index],buf1,READBUF
                     )!=NULL){
                if((
                           file1_line_num/thread_read_block
                   )%gp.threads_num==index){
                    block_line_num1++;
                    if(block_line_num1%4==2){
                        fq1seq.assign(buf1);
                        fq1seq.erase(
...
//                        MDString(seqs[i].c_str(),curData[i]);
        ...
        while(1){
            if(fgets(
                    buf1,READBUF,multi_Nongzfq1[index]
                    )!=NULL){
                if((
                           file1_line_num/thread_read_block
                   )%gp.threads_num==index){
