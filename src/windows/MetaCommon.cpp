...
namespace MetaPreProcessTool {
	...
	void printFqInfo(const string outDir, const string prefix, const FqInfo *fqInfo1, const FqInfo *fqInfo2)
	{
		...
		out << "\n";
		out << "Reads with PolyN (%)                    \t";
		if (fqInfo2 != NULL)
		{
			out << std::left << setw(setwTmp) << fqInfo1->totalPolyNNum * 2 << "\t" << std::right << setw(setwTmp)
				<< setprecision(precisionTmp) << 100.0 * fqInfo1->totalPolyNNum / filteredReads << "%\t";
		}
		out << std::left << setw(setwTmp) << fqInfo1->polyNNum << "\t" << std::right << setw(setwTmp)
			<< setprecision(precisionTmp) << 100.0 * fqInfo1->polyNNum / filteredReads << "%";
		if (fqInfo2 != NULL)
		{
			out << "\t" << std::left << setw(setwTmp) << fqInfo2->polyNNum << "\t" << std::right << setw(setwTmp)
				<< setprecision(precisionTmp) << 100.0 * fqInfo2->polyNNum / filteredReads << "%";
		}
		out << "\n";
